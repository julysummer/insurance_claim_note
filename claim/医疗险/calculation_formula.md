...
| 类型 | key | 示例值 | 说明 |
|------|-----|--------|------|
| 项目分类 | 西药费、中成药费、中药饮片、检查费... | 20000 | 按发票明细项目分类 |
| 医院等级 | 三级、二级、一级、未定级 | 50000 | 按医院等级 |
| 就诊类型 | 门诊、住院 | 100000 | 按就诊方式 |
| 医保结算 | 是、否 | 50000 | 按是否医保结算 |
...
对每个费用明细:
    项目分类 = item.get("项目分类")
    项目金额 = item.get("金额")
    项目限额 = item_limits.get(项目分类, INF)
    项目可赔 = MIN(项目金额, 项目限额) × 赔付比例
...
**方式二：按医院等级**
```
医院等级 = 发票.org_type  // 三级/二级/一级/未定级
如果 医院等级 in item_limits:
    限额 = item_limits[医院等级]
否则:
    限额 = INF
```
...
```
已结算 = 发票.是否医保 == "是"
医保内 = 已结算 ? 个人现金支付 : 发票总金额
医保外 = 已结算 ? 0 : 发票总金额

可赔 = MIN(医保内, 限额.是) + MIN(医保外, 限额.否)
```
