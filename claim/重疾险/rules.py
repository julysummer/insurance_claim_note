...
class ClaimInfo:
    ...
    disease_code: str          # 疾病编码（ICD-10）
    diagnosis_date: date       # 确诊日期
    diagnosis_hospital: str   # 确诊医院
    report_date: date         # 报案日期
    diagnosis_method: str = "" # 确诊方式：病理/影像/临床
    claims_count: int = 0     # 已赔付次数
...
class CriticalIllnessClaimEngine:
    """重疾险理赔规则引擎"""
    ...
    def check_all_rules(self, claim_info: ClaimInfo) -> ClaimResult:
        ...
        # rule_002: 保障期间校验
        if claim_info.diagnosis_date < policy.policy_start_date:
            rejection_reasons.append("确诊日期早于保单生效日期")
            result.rule_results.append(RuleResult("rule_002", "保障期间", False))
        elif claim_info.diagnosis_date > policy.policy_end_date:
            rejection_reasons.append("确诊日期晚于保单终止日期")
            result.rule_results.append(RuleResult("rule_002", "保障期间", False))
        else:
            result.rule_results.append(RuleResult("rule_002", "保障期间", True))

        # rule_003: 等待期校验
        waiting_period = params.get("disease_waiting_period", 90)
        days = (claim_info.diagnosis_date - policy.policy_start_date).days
        if days < waiting_period:
            rejection_reasons.append(f"投保后仅{days}天，等待期需{waiting_period}天")
            result.rule_results.append(RuleResult("rule_003", "等待期", False))
        else:
            result.rule_results.append(RuleResult("rule_003", "等待期", True))

        # rule_004: 报案时效校验
        reporting_days = params.get("reporting_days", 30)
        if (claim_info.report_date - claim_info.diagnosis_date).days > reporting_days:
            rejection_reasons.append(f"报案时间距确诊日期已超过{reporting_days}天")
            result.rule_results.append(RuleResult("rule_004", "报案时效", False))
        else:
            result.rule_results.append(RuleResult("rule_004", "报案时效", True))

        # ========== 第二阶段：疾病校验 ==========
        # rule_005: 确诊信息校验
        if not claim_info.diagnosis_hospital:
            rejection_reasons.append("缺少确诊医院")
            result.rule_results.append(RuleResult("rule_005", "确诊信息", False))
        else:
            result.rule_results.append(RuleResult("rule_005", "确诊信息", True))

        # ========== 第三阶段：赔付次数校验 ==========
        # rule_006: 赔付次数校验
        max_claims = params.get("max_claims", 1)
        if claim_info.claims_count >= max_claims:
            rejection_reasons.append(f"已赔付{claim_info.claims_count}次，达到最高{max_claims}次")
            result.rule_results.append(RuleResult("rule_006", "赔付次数", False))
        else:
            result.rule_results.append(RuleResult("rule_006", "赔付次数", True))
    ...
    def _calculate_payment(self, claim_info: ClaimInfo) -> tuple:
        ...
        - 最高赔付次数: policy.product_params.maxClaims
        ...
        calculation = {
            ...
            "已赔付次数": claim_info.claims_count,
            "最高赔付次数": params.get("max_claims", 1),
